...
double sensor_noise_ultrasonic =
    3.0;  // Change the value of sensor noise to get different KF performance
...
float rotationThreshold =
    1.5;  // because of gyro drifting, defining rotation angular velocity less
          // than this value will not be ignored
float gyroRate = 0;  // read out value of sensor in voltage
// float currentAngle = 0;         // current angle calculated by angular
// velocity integral on
...
double sensor_noise_ir_right =
    1;  // Change the value of sensor noise to get different KF performance
  ...
  if (millis() - previous_millis > T) {
    ...
    if (counter > 10) {
      ...
      if (leftVoltage_nokalman > 0.4 && rightVoltage_nokalman > 0.4) {
        if (abs(leftVoltage_nokalman - rightVoltage_nokalman) < 0.4) {
          stop();
          counter = 0;
...
STATE drive_to_fire() {
  static unsigned long previous_millis;
  // static int currentAngle = START_ANGLE;  // Preserve angle between
  // iterations
  static unsigned long counter = 0;
  ...
  if (millis() - previous_millis > T) {
    ...
    if (counter > 20) {
      ...
      ReadRightFront();
      ReadUltrasonic();
      float average = (leftVoltage_nokalman + rightVoltage_nokalman) / 2;
      if ((frontLeftDist < 13 || frontRightDist < 13 || front_distance < 13) &&
          (average < 3.3)) {
        Serial.print("leftVoltage  ");
        Serial.print(leftVoltage_nokalman);
        Serial.print("rightVoltage  ");
      ...
      if (difference > 0) {
        currentAngle =
            constrain(currentAngle + ANGLE_INCREMENT, SERVO_MIN, SERVO_MAX);
      } else {
        currentAngle =
            constrain(currentAngle - ANGLE_INCREMENT, SERVO_MIN, SERVO_MAX);
      }
      ...
      if (frontLeftDist > 13 && frontRightDist > 13 && front_distance > 13) {
        ...
        // Serial.println(rightPhotoVoltage);
        speed_val = 150;
        left_font_motor.writeMicroseconds(constrain(
            1500 + speed_val - correction_factor, MOTOR_MIN, MOTOR_MAX));
        left_rear_motor.writeMicroseconds(constrain(
            1500 + speed_val - correction_factor, MOTOR_MIN, MOTOR_MAX));
        right_font_motor.writeMicroseconds(constrain(
            1500 - speed_val - correction_factor, MOTOR_MIN, MOTOR_MAX));
        right_rear_motor.writeMicroseconds(constrain(
            1500 - speed_val - correction_factor, MOTOR_MIN, MOTOR_MAX));
...
STATE Avoid() {
  ...
  if (millis() - previous_millis >
      T) {  // Arduino style 100ms timed execution statement
    previous_millis = millis();
    counter++;
    ...
    if ((counter>10) ||(fire_extinguished==0)){
    if (frontLeftDist < 10) {
      stop();
      Serial.println("front left hit");
      counter = 0;
      if (fire_extinguished ==0) {
      return RIGHT;
      } else {
        return LEFT;
      }
...
STATE left() {
  ...
  if (millis() - previous_millis >
    ...
    if ((frontLeftDist > 20) && (frontRightDist > 20) &&
        (front_distance > 20)) {
      counting_time++;
      if (counting_time > 10) {
        counting_time = 0;
...
STATE extinguish() {
  ...
  static unsigned long fire_counter = 0;
  ...
  if (millis() - previous_millis > T) {
    ...
    if (fire_counter < 30) {
      ...
      // float rightVoltage = readPhotoTransistor(RIGHT_PHOTOTRANSISTOR);
      // float leftVoltage_nokalman = readPhotoTransistor(LEFT_PHOTOTRANSISTOR);
      // float rightVoltage_nokalman =
      // readPhotoTransistor(RIGHT_PHOTOTRANSISTOR); ReadPhotoLeft();
      // ReadPhotoRight();
      // Serial.print("leftVoltage: ");
      // Serial.print(leftVoltage_nokalman);
      // Serial.print("  rightVoltage: ");
      // Serial.println(rightVoltage_nokalman);
      if (leftVoltage_nokalman < 0.40 && rightVoltage_nokalman < 0.40) {
        fire_extinguished += 1;
...
//   } else {
//     shortleftIrDistance = 11.35795124 * pow(voltage, -0.7897);  // normal
//     calculation
//   }
...
void ReadLeft() {
  ...
  if (voltage < 0.1) {
    ...
    shortleftIrDistance =
        11.35795124 * pow(voltage, -0.7897);  // normal calculation
  }
...
//   // Calculate the distance using the voltage (for Sharp GP2Y0A21YK0F, this
